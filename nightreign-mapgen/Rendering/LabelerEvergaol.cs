...
{
    /// <summary>
    /// Labels for Evergaol points. Matches names starting with "Evergaol - ".
    /// Uses the "poiStandard" text style and supports multi-line labels with \n.
    /// Offsets configurable at appsettings.json:
    /// "LabelOffsets": { "Evergaol": { "dx": 0, "dy": 0 } }
    /// </summary>
    public static class LabelerEvergaol
    {
        ...
        private static Dictionary<string, (int dx, int dy)> ReadOffsets(JsonElement root)
        {
            ...
            if (root.TryGetProperty("LabelOffsets", out var lo) && lo.ValueKind == JsonValueKind.Object &&
                lo.TryGetProperty("Evergaol", out var eo) && eo.ValueKind == JsonValueKind.Object)
            {
                foreach (var kv in eo.EnumerateObject())
                {
                    var name = kv.Name;
                    var val = kv.Value;
                    try
