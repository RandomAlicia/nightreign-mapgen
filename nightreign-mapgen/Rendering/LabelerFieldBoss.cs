    ...
    public static class LabelerFieldBoss
    {
        ...
        )
        {
            ...
            bool shiftAllowed = poiList.Any(p => {
                var s = select(p);
                return Math.Round(s.x, 1) == 336.0 && Math.Round(s.z, 1) == -46.0;
            });
            ...
            foreach (var poi in poiList)
            {
                ...
                bool isTarget = (Math.Round(x0, 1) == 406.2 && Math.Round(z0, 1) == -10.3);
                ...
                if (!string.IsNullOrEmpty(anchorStr))
                {
                    ...
                    if (anch == "right")      cx -= measureWidth() / 2;
                    else if (anch == "left")  cx += measureWidth() / 2;
                }
        ...
        private static bool TryFindCoordOverride(List<(double x, double z, int dx, int dy, string? style)> list,
                                                double x, double z, double eps,
                                                out (int dx, int dy, string? style) match)
        {
