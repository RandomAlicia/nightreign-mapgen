...
namespace NightReign.MapGen
{
    public static class EvergaolRenderer
    {
        ...
        {
            if (pattern.pois == null || pattern.pois.Count == 0) { if (NightReign.MapGen.Program.Verbose) Console.WriteLine("[Evergaol] No POIs."); return; }
            if (cfg.Evergaol == null || cfg.Evergaol.Default == null) { if (NightReign.MapGen.Program.Verbose) Console.WriteLine("[Evergaol] Config missing (Evergaol.Default)."); return; }
